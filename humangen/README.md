...
The inputs are the robot-free `data/examples/so101_libero_basket/episode_XXX/frames/first.png` stills, from the angled overhead camera with the robot on the right. The hands must enter where the robot stood, so the prompt is:

```text
Convert this image into a photorealistic view of the same scene from the same camera. Keep the camera angle, framing, and every object exactly where it is. Keep these objects unchanged: the white-lined wicker basket, the alphabet soup can, the tomato sauce can, the blue cream cheese box, the ketchup bottle, the wooden table, and the floor. Add a person's two bare forearms and hands reaching in over the near edge of the table from the lower right corner of the frame, as if the person stands just off the lower right of the table facing into it. Both hands rest open on the table near the lower right in a relaxed ready pose, not touching any object. No robot, no extra limbs, no duplicated or missing objects, no text, no overlays. Keep the 4:3 aspect ratio. Do not add or remove objects. Essentially, we only want to add hands, nothing else changed in the scene at all.
```

Save each result as `data/examples/humangen_so101_libero_basket/frames/episode_XXX.png`, then run `uv run python -m humangen data/examples/humangen_so101_libero_basket --aspect 4:3`.
